...
def gconnect():
    ...
    if request.args.get('state') != login_session['state']:from flask import Flask , render_template , request , redirect , jsonify , url_for , flash
app = Flask(__name__)
...
def newItem():
  ...
  if 'username' not in login_session:
      return  render_template('publiccatalog.html', catagory = catagory,items=items)
        ...
        if request.method == 'POST':
            newItem = Item(name = request.form['name'] , description = request.form['des'] , category_name = request.form['cat'] , user_id = user.id)
...
def deleteItem(item_name):
    ...
    if 'username' not in login_session:
        ...
        cat = session.query(Category).filter_by(name = itemToDelete.category_name).one()
